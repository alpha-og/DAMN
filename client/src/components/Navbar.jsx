            ...
            <nav className=" flex flex-row items-center h-12 px-5 mx-5 rounded-2xl bg-slate-900 bg-opacity-60 backdrop-blur-md">
                ...
                <ul className="md:flex hidden flex-row ml-auto ">
                    {routes.map((route, index) => (
                        <li
                            className="flex flex-row ml-2 rounded-lg hover:cursor-pointer hover:text-violet-600 hover:bg-slate-900 hover:shadow-sm hover:shadow-violet-500 ease-in-out duration-500"
                            key={index}
                        >
                ...
                <AiOutlineMenu
                    size={28}
                    className="md:hidden block p-1 ml-auto hover:cursor-pointer hover:text-violet-600 ease-in-out duration-500 ease-in-out duration-500"
                ...
                >
                    <div className="flex flex-row items-center pl-5">
                        ...
                        <AiOutlineClose
                            size={28}
                            className="p-1 ml-auto hover:text-violet-600 ease-in-out duration-500"
                            onClick={toggleNavrail}
                        />
                    </div>
                    ...
                    <ul className="mt-10 flex flex-col">
                        {routes.map((route, index) => (
                            <li
                                className="flex flex-row w-full mb-5 rounded-lg hover:cursor-pointer hover:text-violet-600 hover:bg-slate-900 hover:shadow-sm hover:shadow-violet-500 ease-in-out duration-500"
                                key={index}
                            >
                                <Link
